...
- **Horizon and discretisation.** The lit review names acados SQP-RTI and a 5–20 ms solve target but does not commit to a horizon length or shooting node count. Pick during phase 3 of the research plan. *Defaulted (not committed) in the 03 prototype: `N = 40`, `tf = 4.0 s` (`dt = 0.1 s`), tunable per experiment config.*
...
- `hilda_nmpc/` — standalone acados OCP prototype (6-state SQP-RTI, soft `f ≥ 0` via `cs.f`) landed 2026-05-31; closes "horizon/discretisation" *with a default* (`N=40`, `tf=4.0 s`), keeps NMPC/HMPC, 6/7-state, soft/hard, IRM all open. Constraint config is two orthogonal axes (mode × barrier, [ADR 0014](decisions/0014-ocp-constraint-config-axes.md)); barrier-on / Baseline C reserved. Validated by `sim_validation_03{a,b,c}` (transit / fixed-`s` ablation / drill-target) — all PASS. Design: [spec](superpowers/specs/2026-05-31-acados-ocp-prototype-design.md).
