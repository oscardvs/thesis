...
- **03b — fixed-`s` ablation.** Same scene as 03a, `s` frozen at `s_nominal` (`u_s ≡ 0`, `s` removed from the decision). The experiment **passes its hypothesis when fixed-`s` cannot reach the goal while keeping clearance** — `hypothesis_confirmed = not(reach and feasible)`. *Refined during execution (see [ADR 0014](../../decisions/0014-ocp-constraint-config-axes.md)): with a high slack penalty `zl`, fixed-`s` does not violate `f` — it **stalls** at the beam edge (~2.24 m short of the goal, staying ~feasible). The not-reach-while-feasible criterion is robust to stall or violation.* Same `closed_loop`/`gates` code as 03a; the verdict is the joint-optimisation evidence. Citable contrast: joint-opt reaches the goal by lowering `s` (min clearance +0.138 m); fixed-`s` stalls.
